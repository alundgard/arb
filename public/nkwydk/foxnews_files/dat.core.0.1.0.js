!function(A){
    ...
    var makeCallsThenContinue = function(urlList,callback){
        ...
        var _onCallComplete = function(){
            count++;
            log('completed call '+count);
            if (count===urlList.length) callback();
        };
        for(var i=0;i<urlList.length;i++){
            var url = typeof urlList[i].url != "undefined" ? replaceWithData(urlList[i].url) : "";
            log('type:'+urlList[i].type+' url:'+url);
            if (urlList[i].type==='script')
                loadScript(url, _onCallComplete, _onCallComplete);//make all async calls
            else if (urlList[i].type==='callback') {
                var args = urlList[i].args.slice();
                args.push(_onCallComplete);
                urlList[i].func.apply(this, args);
            }else if (urlList[i].type==='image') loadImage(url, _onCallComplete, _onCallComplete);
            else _onCallComplete();
        };
    };
